...
graph TB
    ...
    subgraph PermSystem["权限系统"]
        ...
        PR["PermissionRule<br/>─────────────<br/>tool, glob/regex<br/>ruleBehavior: allow|deny|ask"]
        PRes["PermissionResult<br/>─────────────<br/>behavior: allow|deny|ask<br/>decisionReason"]
        PC --> PR
        PC --> PRes
    end
    ...
    classDef react fill:#16213e,stroke:#0f3460,color:#fff
    classDef state fill:#0f3460,stroke:#533483,color:#fff
    classDef query fill:#533483,stroke:#e94560,color:#fff
    classDef tool fill:#2b2d42,stroke:#8d99ae,color:#fff
    classDef perm fill:#5c2a2a,stroke:#c0392b,color:#fff
    classDef bridge fill:#1a3a3a,stroke:#1abc9c,color:#fff
    classDef mcp fill:#2d2a5c,stroke:#9b59b6,color:#fff
    classDef task fill:#3a2a1a,stroke:#e67e22,color:#fff
    classDef cost fill:#1a3a1a,stroke:#27ae60,color:#fff
...
graph TB
    subgraph Priority["配置优先级 (从高到低)"]
        direction TB
        P1["① 环境变量<br/><code>CLAUDE_CODE_*</code><br/><i>最高优先级</i>"]
        P2["② MDM / 远程策略<br/><code>managed-settings.json</code><br/><code>remote-settings.json</code><br/><i>企业管控</i>"]
        P3["③ 项目本地配置<br/><code>.claude/settings.local.json</code><br/><i>不入版本控制</i>"]
        P4["④ 项目共享配置<br/><code>.claude/settings.json</code><br/><i>团队共享，入版本控制</i>"]
        P5["⑤ 用户本地配置<br/><code>~/.claude/settings.local.json</code><br/><i>用户私有覆盖</i>"]
        P6["⑥ 用户全局配置<br/><code>~/.claude/settings.json</code><br/><i>用户默认值</i>"]
        P1 --> P2 --> P3 --> P4 --> P5 --> P6
    end
...
graph TB
    ...
    subgraph ChildProcesses["子进程群"]
        direction TB
        BashChild["Bash 子进程<br/><code>spawn('bash', ['-c', cmd])</code><br/>─────────────<br/>用户命令执行<br/>stdout/stderr 管道<br/>沙盒隔离 (macOS sandbox-exec /<br/>Linux Bubblewrap)"]

        AgentChild["Agent 子进程<br/><code>fork()</code><br/>─────────────<br/>独立 Node.js 进程<br/>拥有自己的消息历史<br/>通过 IPC Channel 通信"]

        AgentChild2["Agent 子进程 N<br/><code>fork()</code><br/><i>可并发多个</i>"]

        TeammateChild["Teammate 子进程<br/><code>in_process_teammate</code><br/>─────────────<br/>进程内子代理<br/>共享部分上下文"]
    end

    subgraph MCPServers["MCP Server 进程群"]
        direction TB
        MCP1["MCP Server A<br/><code>spawn(command, args)</code><br/>─────────────<br/>stdio 传输<br/>JSON-RPC 2.0"]
    ...
    classDef main fill:#1a1a2e,stroke:#e94560,color:#fff
    classDef child fill:#16213e,stroke:#0f3460,color:#fff
    classDef mcp fill:#2d2a5c,stroke:#9b59b6,color:#fff
    classDef net fill:#1a3a3a,stroke:#1abc9c,color:#fff
    classDef event fill:#0f3460,stroke:#533483,color:#fff
